...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    ...
    runAsynchronouslyOnVideoProcessingQueue(^{
        ...
        void(^write)() = ^() {
            ...
            {                
                BOOL result = [assetWriterPixelBufferInput appendPixelBuffer:pixel_buffer withPresentationTime:frameTime];
                if (frameWrittenCompletionBlock) {
                    NSLog(@"Frame written with pts %f", CMTimeGetSeconds(frameTime));
                    frameWrittenCompletionBlock(result, frameTime);
                }
                if (!result) {
